...
class AIAnalyzer:
    ...
    def __init__(self, config: Dict):
        ...
        self.pandas_ai = PandasAI(
            ...
            cache_dir=config.get('cache_dir', '.cache/pandasai'),
        ...
        # 创建缓存目录
        os.makedirs(config.get('cache_dir', '.cache/pandasai'), exist_ok=True)
    
    @lru_cache(maxsize=100)
    def _get_cached_analysis(self, query: str, prompt: str) -> Dict:
        """获取缓存的分析结果
        ...
        return self.pandas_ai.run(df, prompt)
    ...
    def _get_default_prompts(self) -> Dict:
        ...
        return {
            'trend_analysis': """
            分析{celebrity}在{time_range}内的{metric}趋势，并解释关键变化点。
            请提供以下分析：
            1. 整体趋势走向
            2. 关键峰值和谷值
            3. 异常值分析
            ...
            """,
            'sentiment_analysis': """
            分析{celebrity}的评论情感分布，识别主要情感倾向和变化。
            请提供以下分析：
            1. 情感分布概况
            2. 主要情感类型
            3. 情感变化趋势
            ...
            """,
            'fan_analysis': """
            分析{celebrity}的粉丝画像，包括活跃度、互动类型等特征。
            请提供以下分析：
            1. 粉丝基础特征
            2. 活跃度分析
            3. 互动偏好
            ...
            """,
            'comparison_analysis': """
            比较{celebrity1}和{celebrity2}在{metric}方面的差异。
            请提供以下分析：
            1. 整体对比
            2. 关键指标对比
            3. 优势劣势分析
