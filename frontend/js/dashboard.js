    ...
    grid.innerHTML = `
        ...
        ${stars.map(star => `
            <div class="star-card ${state.selectedStars.has(star.id) ? 'selected' : ''}" 
                 onclick="${state.isBatchMode ? `toggleStarSelection(${star.id}, event)` : `window.location.href='/star_detail.html?star_id=${star.id}'`}">
...
async function fetchAllStarsData() {
    try {
        ...
        console.log('获取到的明星数据:', data);
        
        if (response.ok) {
            if (Array.isArray(data)) {
                updateStarsGrid(data);
            } else if (data.stars && Array.isArray(data.stars)) {
                updateStarsGrid(data.stars);
            } else {
                throw new Error('服务器返回的数据格式不正确');
            }
        } else {
            throw new Error(data.error || '获取明星列表失败');
        }
    } catch (error) {
        console.error('获取明星列表错误:', error);
...
async function handleAddStar(event) {
    ...
    const formData = {
        ...
        star_url: document.getElementById('starUrl').value
    };
