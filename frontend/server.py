...
from api.datacrawl.datacrawl import WeiboSpiderRunner
...
def delete_star(star_id):
    ...
    try:
        ...
        try:
            # 1. 获取要删除的明星信息
            star = session.query(Celebrity).filter(Celebrity.weibo_id == str(star_id)).first()
            if not star:
                return jsonify({'error': '明星不存在'}), 404
                
            # 2. 删除与该明星相关的所有数据
            # 2.1 删除评论数据
            session.query(Comment).join(Post).filter(Post.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            
            # 2.2 删除微博数据
            session.query(Post).filter(Post.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            
            # 2.3 删除用户-明星关联
            session.query(UserCelebrity).filter(UserCelebrity.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            
            # 2.4 删除热度数据
            session.query(HeatData).filter(HeatData.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            
            # 2.5 删除情感分析数据
            session.query(SentimentForPost).join(Post).filter(Post.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            session.query(SentimentForComment).join(Comment).join(Post).filter(Post.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            
            # 2.6 删除黑粉分析数据
            session.query(BlackFanAnalysis).filter(BlackFanAnalysis.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            session.query(BlackFan).filter(BlackFan.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            
            # 2.7 删除活跃粉丝数据
            session.query(ActiveFan).filter(ActiveFan.celebrity_id == star.weibo_id).delete(synchronize_session=False)
            ...
            # 3. 更新内存中的明星数据
            global stars_data
            stars_data = [star for star in stars_data if star['id'] != star_id]
        ...
        return jsonify({'error': '删除明星数据失败'}), 500
